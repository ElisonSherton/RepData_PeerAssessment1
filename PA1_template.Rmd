...
**The interval `r nstepmax` has maximum number of steps on an average across all the days.**
...
```
**Before imputing, mean = `r meansteps`, median = `r mediansteps` and after imputing, mean = `r newmeansteps` and median = `r newmediansteps`. This means that after central imputation mean has remained unchanged (since we imputed with the mean) but new median has risen compared to the median before imputation.**
